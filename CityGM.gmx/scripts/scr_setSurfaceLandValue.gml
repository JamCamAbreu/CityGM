...
if (surface_exists(argument0.surfaceSpecialMap)) {
    ...
    var landValueString = DLL_tileDataToString(TDT_LANDVALUE);
    ...
    for (r = 0; r < MAP_DIMENSION; r++) {
        for (c = 0; c < MAP_DIMENSION; c++) {    
            ...
            valString = ""; // reset
            if (stringPos <= string_length(landValueString)) {
                while (string_char_at(landValueString, stringPos) != ',') {
                    valString += string_char_at(landValueString, stringPos);
